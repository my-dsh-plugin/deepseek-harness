...
The restriction to an idle agent is a product rule, not a mechanical one: swapping tools mid-turn would change the tool schemas the running request was assembled against, and history produced under the previous composition may name tools the new one cannot make. The gateway enforces the running-turn part at the wire ([`dsh-apiproxy`](../../host/apiproxy/README.md) answers `agent-preset-locked` while the agent runs); the history trade-off of a started-conversation switch is the caller's to own — the `dsh-agent-mode-switcher` plugin offers exactly that.
...
- **A preset cannot be changed while a session runs a turn** — `recompose` re-links an IDLE session's parent scope to another standing mount; a swap mid-answer would change the tool schemas the running request was assembled against. Switching a started conversation is allowed and leaves history produced under the previous composition as the caller's trade-off.
