...
- **A session cannot change preset while it is running a turn** — switching re-links the session's parent scope to another standing mount, and only an idle session: swapping tools mid-turn would strand tools the model has called. Once the turn ends, the log's `agent-preset/selected` entry records the switch and the next turn runs under the new composition.
